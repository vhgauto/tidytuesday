...
write_tsv(datos_saved, here("2023-s12/datos_saved.tsv"))
datos_saved <- read_tsv(here("2023-s12/datos_saved.tsv"))
...
g1 <- ggplot(data = datos, aes(x = visitas, y = nombre)) +
  ...
  geom_point(aes(shape = decada), size = 1, color = "white", fill = "white") +
  ...
  # ejes
  labs(x = "Visitas diarias a la entrada en Wikipedia del lenguaje", y = NULL,
  ...
  theme(aspect.ratio = .25,
        ...
        strip.text.y.right = element_markdown(angle = 0, color = "#003354",
                                              family = "heebo", vjust = .5))

# guardo
ggsave(plot = g1,
       filename = here("2023-s12/viz.png"),
       width = 19,
       height = 27,
       dpi = 300,
       units = "cm")
# abro
browseURL(here("2023-s12/viz.png"))
